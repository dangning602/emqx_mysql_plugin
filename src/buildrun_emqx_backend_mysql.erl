...
-define(MESSAGE_PUBLISH_SQL_JSON,
                 <<"insert into mqtt_JSON(msgid, sender, "
                   "topic, qos, retain, payload, arrived, content) "
                   "values (?, ?, ?, ?, ?, ?, ?, ? );">>).  
...
on_message_publish(#message{flags = #{retain := true}} = Message, _Env) ->
    ...
    Temp = jiffy:decode(Payload,[return_maps]),
    io:format("C1 ~w~n",[Temp]),
    Value = maps:get(<<"msg">>,Temp),
    io:format("C ~w~n",[Value]),
    buildrun_emqx_backend_mysql_cli:query(?MESSAGE_PUBLISH_SQL_JSON, [emqx_guid:to_hexstr(Id),binary_to_list(From),Topic,integer_to_list(Qos),null,binary_to_list(Payload),null,Value]),
    {ok, Message};
